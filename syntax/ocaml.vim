...
" if exists("ocaml_noend_error") || exists("ocaml_end")
  syn match    ocamlKeyword    "\<end\>"
" else
  " syn match    ocamlEndErr     "\<end\>"
" endif
  syn match    ocamlKeyword    "\<debug\>"

if exists("alphaCaml")
  syn match    ocamlKeyword    "\<\(binds\|sort\|inner\|outer\|neutral\|atom\)\>"
endif


" Some convenient clusters
syn cluster  ocamlAllErrs contains=ocamlBraceErr,ocamlBrackErr,ocamlParenErr,ocamlCommentErr,ocamlCountErr,ocamlDoErr,ocamlDoneErr,ocamlEndErr,ocamlThenErr,ocamlParseErr,ocamlQuotaErr

syn cluster  ocamlAENoParen contains=ocamlBraceErr,ocamlBrackErr,ocamlCommentErr,ocamlCountErr,ocamlDoErr,ocamlDoneErr,ocamlEndErr,ocamlThenErr,ocamlParseErr,ocamlQuotaErr
...
"syn region   ocamlEncl transparent matchgroup=ocamlKeyword start="\[<" matchgroup=ocamlKeyword end=">\]" contains=ALLBUT,@ocamlContained
syn region   ocamlEncl transparent matchgroup=ocamlQuota start="<\(\|:[\w@]*\)<" matchgroup=ocamlQuota end=">>" contains=ALLBUT,@ocamlContained
...
" OCaml end
if exists("ocaml_end")
  " syn region   ocamlEnd matchgroup=ocamlMatch start="\<match\>" matchgroup=ocamlMatch end="\<end\>" contains=ALLBUT,@ocamlContained,ocamlEndErr
  " syn sync     match    ocamlEndSync          grouphere ocamlEnd "\<\(match\)\>"
  " syn sync     match    ocamlMatchSync  grouphere  ocamlMatch  "\<match\>"
  " syn sync     match    ocamlMatchSync  groupthere ocamlMatch  "\<end\>"
endif
...
syn keyword ocamlKeyword module open functor
...
if exists("ocaml_revised")
  syn keyword  ocamlKeyword  value declare
...
syn match    ocamlQuota      "<:\(\l\|_\)\(\w\|'\|@\)*<"
syn match    ocamlQuota      "<<"
syn match    ocamlQuota      ">>"
syn match    ocamlOperator   ">>>"
syn match    ocamlOperator   "<<<"
...
if exists("ocaml_end")
  ...
  HiLink ocamlQuota        Identifier
  HiLink ocamlApp          Identifier
...
" syn region Function_Folding start="\<\(struct\|sig\)\>" end="\<end;" transparent fold
" syn sync fromstart
